"""
VoiceAI 2.0 - Production Ready Application
Clean, scalable, and production-ready voice AI system
"""
...
from config.settings import get_settings, print_config_summary
...
# Initialize systems
appointment_manager = AppointmentManager() if settings.enable_calendar_integration else None
...
class ProductionVoiceAI:
    """Production-ready VoiceAI system"""
    
    def __init__(self):
        self.system_prompt = f"""You are a professional dental assistant AI for Demo Dental Practice. 
...
Always end successful bookings with a confirmation number."""
    ...
    def extract_patient_info(self, conversation_history: list) -> Dict[str, Any]:
        """Extract patient information from conversation using improved parsing"""
        info = {
            "name": None,
            "phone": None,
        ...
        name_patterns = [
            ...
            r"([a-z]+\s+[a-z]+)(?:\s+speaking|\s+here|\s*$)"
        ]
        ...
        # Extract appointment type with confidence scoring
        ...
        time_keywords = {
            ...
            "afternoon": ["afternoon", "pm", "later"],
            "specific_time": [r"\d{1,2}:\d{2}", r"\d{1,2}\s*(?:am|pm)"]
        }
        
        for time_pref, patterns in time_keywords.items():
            for pattern in patterns:
                if re.search(pattern, full_text, re.IGNORECASE):
                    info["preferred_time"] = time_pref
                    info["confidence"] += 0.2
                    break
    ...
    async def generate_ai_response(self, user_input: str, call_id: str) -> str:
        ...
        if client and settings.openai.is_configured:
            try:
    ...
    async def generate_rule_based_response(self, user_input: str, call_id: str) -> str:
        ...
        # Determine response based on extracted information and conversation state
        if not history or "appointment" not in conversation_text.lower():
            ...
            if patient_info["urgency"] == "urgent":
                ...
                if appointment_manager:
                    ...
                    if available_slots:
                        response = f"I have availability at {', '.join(available_slots)}. Which works for you?"
                    else:
...
async def health_check():
    ...
    health_data = {
        ...
        "configuration": settings.system_status,
        "stats": system_stats,
        "features": {
            "ai_integration": settings.openai.is_configured,
            "calendar_integration": settings.enable_calendar_integration,
...
async def voice_webhook(
    ...
    """Production voice webhook with intelligent routing"""
    ...
    try:
        # Determine if we should use ConversationRelay
        use_relay = (
            settings.enable_conversation_relay and 
        ...
        if use_relay:
            # ConversationRelay for production
...
<Response>
    ...
    <Record timeout="5" maxLength="30" action="/api/v1/voice/{tenant_id}/process" playBeep="true"/>
            ...
            elif CallStatus in ["ringing", "in-progress"]:
...
async def list_appointments():
    ...
    return {
        ...
        "calendar_integrated": appointment_manager is not None
    }
...
if __name__ == "__main__":
    print("Starting VoiceAI 2.0 - Production Application")
    print_config_summary(settings)
